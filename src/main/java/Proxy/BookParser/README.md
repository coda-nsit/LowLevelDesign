...
the `BookParser` directly calls the `BookParserProxy`, which implements the same interface as `BookParser`. 
`BookParserProxy` introduces additional functionality for example,
1. authentication
